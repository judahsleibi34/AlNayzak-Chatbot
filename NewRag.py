...
- If RET.SANITY_PROMPTS is missing, falls back to DEFAULT_SANITY_PROMPTS.
...
def ask_once(index: RET.HybridIndex, tokenizer, model, question: str, use_llm: bool = True) -> str:
    t0 = time.time()
    ...
    if intent in ("work_hours", "ramadan_hours") and ("ساعات الدوام" in extractive_answer) and ("من" in extractive_answer) and ("إلى" in extractive_answer):
        dt = time.time() - t0
        return extractive_answer if extractive_answer.startswith("⏱") else f"⏱ {dt:.2f}s | 🤖 {extractive_answer}"
    ...
    try:
        from transformers import AutoTokenizer, AutoModelForCausalLM  # just to ensure installed
        ...
        out_ids = model.generate(**inputs, max_new_tokens=200, temperature=0.1, do_sample=False, eos_token_id=eos_id, pad_token_id=pad_id)
        ...
        logging.getLogger("rag_orchestrator").warning(f"LLM generation failed: {e}")
    dt = time.time() - t0
    return extractive_answer if str(extractive_answer).startswith("⏱") else f"⏱ {dt:.2f}s | 🤖 {extractive_answer}"
...
def run_test_prompts(index: RET.HybridIndex, tokenizer, model, use_llm: bool):
    ...
    if not tests:
        print("❌ No sanity prompts available."); return
    print("🧪 Running sanity prompts ..."); print("=" * 80)
    passed = 0
    for i, q in enumerate(tests, 1):
        print(f"\n📝 Test {i}/{len(tests)}: {q}"); print("-" * 60)
        try:
            ...
            print("✅ PASS" if ok else "❌ FAIL"); passed += int(ok)
        except Exception as e:
            print(f"❌ Error: {e}")
        print("=" * 80)
    print(f"\nSummary: PASS {passed}/{len(tests)}")
...
def main():
    ...
    ap.add_argument("--test", action="store_true", help="Run sanity prompts (alias: --sanity)")
    ap.add_argument("--sanity", action="store_true", help="Alias for --test")
    ...
    if not loaded:
        ...
        if args.save_index:
            ...
            except Exception as e: LOG.warning(f"Failed to save index: {e}")
    ...
    if use_llm:
        try:
            ...
            dtype_fp16 = torch.bfloat16 if (torch is not None and bf16) else (torch.float16 if torch is not None else None)
            model_kwargs = {"trust_remote_code": True, "torch_dtype": dtype_fp16 or None}
            if torch is not None and hasattr(torch, "cuda") and torch.cuda.is_available():
            ...
            if args.use_4bit or args.use_8bit:
                try:
                    model_kwargs["quantization_config"] = BitsAndBytesConfig(
                        ...
                        bnb_4bit_compute_dtype=(torch.bfloat16 if (torch is not None and bf16) else (torch.float16 if torch is not None else None)),
                    )
                except Exception as e:
    ...
    if args.test or args.sanity:
        run_test_prompts(index, tok, mdl, use_llm=use_llm); return

    if args.ask:
        print(ask_once(index, tok, mdl, args.ask, use_llm=use_llm)); return
